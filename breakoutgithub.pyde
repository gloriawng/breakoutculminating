...
ballSpeedX = -3
ballSpeedY = -3
>>>>>>> cbd9075dceaa56d1cdd730fddfccbb99bc0af560
...
def startScreen():
    ...
    textSize(15)
<<<<<<< HEAD
    text("A brick wall is at the top section of the screen, use the mouse to control the", 500, 300)
    text("paddle and hit the ball upwards. The ball will eliminate the blocks and when all the", 500, 320)
    text("blocks are gone, YOU WIN. You have 3 lives to do so. You lose a life if the ball goes", 500, 340)
    text("past the paddle. Once, all 3 lives are gone, YOU LOSE. The game info will be in", 500, 360)
    text("the top left and music and play/pause button will be in the top right. Hitting", 500, 380)
    text("the corner of blocks will give twice the points and a special bounce sometimes.", 500, 400)
=======
...
def gameScreen():
<<<<<<< HEAD
    global ballX, ballY, ballSpeedX, ballSpeedY, lives, mouseX, displayScreen, gameWon, paddleLeft, bricks, musicPlay, gamePause
...
<<<<<<< HEAD
    ...
    #All the encounters with wall of blocks
    brick()
    ...
    if ballSpeedY > 0:
        if ballY + 9 >= 550:
            ...
            if ballX >= paddleLeft + 100 and ballX <= paddleLeft + 160:
                ...
                if ballSpeedX < 0:
                    ballSpeedX = -ballSpeedX
            if ballX >= paddleLeft + 60 and ballX < paddleLeft + 80:
                ballSpeedY = -ballSpeedY
                ...
                if ballSpeedX == 0:
                    ballSpeedX = -2
            if ballX >= paddleLeft + 80 and ballX < paddleLeft + 100:
                ballSpeedY = -ballSpeedY
                ballSpeedX = -ballSpeedX / 2
                ballY = 541
                if ballSpeedX < 0:
...
<<<<<<< HEAD
            ...
            #Creates the blocks    
            if bricks[i][j] == True:
                rect(j * 100 + (j + 1) * 10, i * 40 + (i + 1) * 10 + 50, 100, 40)

                #Interaction with blocks // Hitting the blocks gives points and hitting the corner of the blocks gives double the amount of points that would have been given
                if ballX + 9 >= j * 100 + (j + 1) * 10 and ballX - 9 <= j * 100 + (j + 1) * 10 and ballY + 9 >= i * 40 + (i + 1) * 10 + 50 and ballY - 9 <= i * 40 + (i + 1) * 10 + 90:
                    ballSpeedX = -ballSpeedX
                    bricks[i][j] = False
                    score += points
                if ballX - 9 <= j * 100 + (j + 1) * 10 + 100 and ballX + 9 >= j * 100 + (j + 1) * 10 + 100 and ballY + 9 >= i * 40 + (i + 1) * 10 + 50 and ballY - 9 <= i * 40 + (i + 1) * 10 + 90:
                    ballSpeedX = -ballSpeedX
                    bricks[i][j] = False
                    score += points
                if ballY + 9 >= i * 40 + (i + 1) * 10 + 50 and ballY - 9 <= i * 40 + (i + 1) * 10 + 50 and ballX + 9 >= j * 100 + (j + 1) * 10 and ballX - 9 <= j * 100 + (j + 1) * 10 + 100:
                    ballSpeedY = -ballSpeedY
                    bricks[i][j] = False
                    score += points
                if ballY - 9 <= i * 40 + (i + 1) * 10 + 90 and ballY + 9 >= i * 40 + (i + 1) * 10 + 90 and ballX + 9 >= j * 100 + (j + 1) * 10 and ballX - 9 <= j * 100 + (j + 1) * 10 + 100:
                    ballSpeedY = -ballSpeedY
                    bricks[i][j] = False
                    score += points
...
def mousePressed():
    global displayScreen, bricks, lives, score, song, musicPlay, gamePause, ballX, ballY, ballSpeedX, ballSpeedY
=======
